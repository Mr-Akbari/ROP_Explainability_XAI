...
def postprocess(probResult,probImage):
    ...
    for region in regionprops:  # Loop to get each connected region attribute set
        ...
        if math.fabs((maxr - minr) / (maxc - minc)) > 1.3 or math.fabs((maxr - minr) / (maxc - minc)) < 0.8 or area * 4/3.1415926 < countWhite(probResult[minr:maxr, minc:maxc]):
            ...
            continue
    #筛选过的区域与已选择区域合 The filtered area is combined with the selected area
...
def recompone_overlap(preds,config,img_h,img_w):
    """
    Stitch the patch into the original picture
    :param preds: patch块
    :param config: 配置文件
