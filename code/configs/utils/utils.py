    ...
    for i in range(row):
        for j in range(col):
            ...
            if index<total_image:
                print("image[index]:", image[index].shape)
                print("len(image[index]):", len(image[index].shape))
                if image[index].shape[-1] !=3:
                    image[index] = np.expand_dims(image[index],axis=2)
                    print("image[index]:", image[index].shape)
                try:  #Single-channel grayscale image and 3-channel color image are processed separately
                    result[row_index:row_index+height,col_index:col_index+width,:]=image[index]*255
                except:
                    result[row_index:row_index + height, col_index:col_index + width, 0] = image[index][:,:,0]*255
                    result[row_index:row_index + height, col_index:col_index + width, 1] = image[index][:,:,1]*255
                    result[row_index:row_index + height, col_index:col_index + width, 2] = image[index][:,:,2]*255
            index=index+1
