...
#    print 'hes is', hes_newX
    ...
    if ~np.isnan(hes_newX).any() and ~np.isnan(grad_newX).any():
        ...
        sortedEigVal, sortedEigVec=list(zip(*sorted(zip(eigVal,np.transpose(eigVec)),reverse=True)))        
...
#    print 'scale took' , time2-time1
    ...
    for i in seeds:
        xStart=i.copy()
        [neighList, neighS, neighW]= list(zip(*[ (mean,S[index], w[index])  for index,mean in enumerate(means) if np.all(np.abs((xStart-mean))<maxS[index])]))#np.linalg.norm(np.dot(S[index],xStart-mean),np.inf)< 5.])#       
